...
namespace AkieEmpty.SkillEditor
{
    ...
    public class SkillEditorWindow : EditorWindow, ISkillEditorWindow
    {
        ...
        private void InitTimeShift()
        {
            ...
            ScrollView mainContentView = Root.Q<ScrollView>("MainContentView") ?? throw new InvalidOperationException("找不到控件: MainContentView");
        ...
        private void UnInitConsole()
        {
            previouFrameButton.clicked -= OnPreviouFrameButtonClick;
            playButton.clicked -= OnPlayButtonClick;
            nextFrameButton.clicked -= OnNextFrameButtonClick;
            currentFrameField.UnregisterValueChangedCallback(OnCurrentFrameFieldValueChanged);
            maxFrameCountField.UnregisterValueChangedCallback(OnMaxFrameCountValueChanged);
        }
